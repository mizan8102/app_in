...
<html>
<head>
    <title>Purchase Order Summery</title>
</head>
<style type="text/css">
    @page {
            header: page-header;
    ...
    <div class="row" style="none">
        ...
        <div class="column3">
            <div class="head_middle">
                ...
                <div style="text-align: center; padding-top:-45px;">
                    <p style="font-size: 12pt;">Purchase Order Summery<br>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div style="margin-left:5px; margin-top:20px; display:block;">
            <div class="report_params_areas" style="float:left;height:20px;width:50%">
                <span style="text-align: right; font-size:12px"><b> Open PO till Date: </span>
            </div>
            ...
            <!-- <div class="report_params_areas" style="float:left;height:20px;width:100%">
                <span style="text-align: right; font-size:12px"><b>PO Date: </b>{{$pOrders[0]->poDate ?? ''}}</span>
                <br>
            </div> -->
        </div>
        ...
        <table width="100%" style="display:block; margin-top: 10px;">
            <thead>
                <tr style="background-color: rgb(3,73,91);">
                    ...
                    <th style="text-align: center; color: rgb(255,255,255);">Delivery Date</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align: center;">{{$data->poNo  ?? ''}}</td>
                    ...
                    <td style="text-align: left;">{{$data->MasterGroupName  ?? ''}}</td>
                    <td style="text-align: left;">{{$data->MasterGroupNameBn  ?? ''}}</td>
                    <td style="text-align: center;">{{$data->deliveryDate  ?? ''}}</td>
                    <td style="text-align: right;">{{number_format($data->poAmount, 2, '.', ',')  ?? ''}}</td>
                    <td style="text-align: center;">{{$data->closeStatus  ?? ''}}</td>
                    <td style="text-align: left;">{{$data->remarks  ?? ''}}</td>
                </tr>
                @endforeach
                <tr style="border-bottom: 1px solid black;">
                    <td style="text-align: right;"colspan ="8"><strong>Total: </strong></td>
                    <td style="text-align: right;"colspan =""><strong>{{number_format(collect($pOrders  ?? '')->sum('poAmount'), 2, '.', ',')}}</strong></td>
