...
<body>
    ...
    <div style="margin-left:3px; margin-top:20px;">
        <p style="text-align: left; font-size: 12px">
            Receive Date: {{ $grn[0]->grnDate }}<br>
            From: {{ $grn[0]->supplierName }}<br>
            Name: {{ $grn[0]->supplierName }}<br>
            Address: {{ $grn[0]->supplierAddress }}<br>
            Mobile No: {{ $grn[0]->supplierPhone }}
        </p>
        <div class="columne" style="text-align:right; float:right; margin-right:10px; font-size: 12px;">

            <p style="padding-top:-35px;"><strong>Print: </strong>
                @php
                $mytime = Carbon\Carbon::now()->format ('d-m-Y h:i A');
                echo $mytime;
                @endphp
                <br> Received By: kamruzzaman Polash
            </p>
        </div>
    </div>
    <table width="100%" style="margin-top:90px;">
        ...
        <tbody>
            @foreach ($grn as $key=>$item)
            <tr>
                <td style="text-align: center;">{{ $key+1 }}</td>
                <td style="text-align: left;">{{ $item->itemName }}</td>
                <td style="text-align: center;">{{ $item->rcvQty }}</td>
                <td style="text-align: center;">{{ $item->uom }}</td>
                <td style="text-align: right;">{{ $item->rcvRate }}</td>
                <td style="text-align: right;">{{ $item->rcvAmount }}</td>
                <td style="text-align: left;">{{ $item->remraks }}</td>
            </tr>
            @endforeach
            <tr style="border-bottom: 1px solid black;">
                ...
                $totalamnt = 0;
                foreach ($grn as $item) {
                $totalqty += $item->rcvQty;
                $totalamnt += $item->rcvAmount;
                }
                @endphp
                <td style="text-align: right;" colspan="2"><strong>Sub Total: </strong></td>
                <td style="text-align: center;"><strong>{{number_format($totalqty, 2, '.', ',')}}</strong></td>
                <td></td>
                <td></td>
                <td style="text-align: right;"><strong>{{number_format($totalamnt, 2, '.', ',')}}</strong></td>
                <td></td>
            </tr>
        </tbody>
    </table>
    <p style="font-size:12px;">{In Words: Nine Thousand Six Hundred Only}</p>
    <div class="note">
        <p>
            **Note: If product category is "Product" then purchase order for Service it will be Service Work Order.
        </p>
        <ol>
            <li>Waiter: Need Registration and Attendance.</li>
            <li>For Repair: Returnable Gate Pass</li>
            <li>Daily Labour: Need Registration and attendance and daily Output Records.</li>
            <li>Make a Receive Note</li>
            </0l>
            <div>
