    ...
    <body>
            ...
            <div class="column3">
                <div class="head_middle">
                    ...
                    <div style="text-align: center; padding-top:-45px;">
                        <p>Receive/GRN Details Report<br>
                        </p>
                    </div>
                </div>
            </div>
            <div class="column1">
                <p style="text-align: right; font-size:12px;">Chiklee#C_03A</p>
            </div>
        </div>
        <div style="margin-left:5px; margin-top:20px; display:block;">
            <div class="report_params_areas" style="float:left;height:20px;width:50%">
                <span style="text-align: right; font-size:12px"><b> Receive Date: </b>{{Date('d-m-Y',strtotime($receives[0]->grnDate ?? ''))}}</span>
            </div>
            ...
            <div class="report_params_areas" style="float:left;height:20px;width:50%">
                <span style="text-align: right; font-size:12px"><b> Receive By: {{$receives[0]->recBy ?? ''}}</b></span>
            </div>
            <div class="report_params_areas" style="float:left;height:20px;width:100%">
                <span style="text-align: right; font-size:12px"><b> From: </b>{{$receives[0]->supplierName ?? ''}}</span>
                <br>
                <span style="text-align: right; font-size:12px"> Address: {{$receives[0]->supplierAddress ?? ''}}</span>
                <br>
                <span style="text-align: right; font-size:12px"> Mobile:    {{$receives[0]->supplierPhone ?? ''}}</span>
                <br>
            ...
            <tbody>
                @foreach($receives as $key => $receive)
                    <tr>
                        ...
                        <td style="text-align: left;">{{$receive->itemName}}</td>
                        <td style="text-align: left; font-size:14px;">{{$receive->itemNameBn}}</td>
                        <td style="text-align: center;">{{number_format($receive->rcvQty, 2, '.', ',')}}</td>
                        <td style="text-align: center;">{{$receive->uom}}</td>
                        <td style="text-align: right;">{{number_format($receive->rcvRate, 2, '.', ',')}}</td>
                        <td style="text-align: right;">{{number_format($receive->rcvAmount, 2, '.', ',')}}</td>
                        <td style="text-align: left;"></td>
                    </tr>
                @endforeach
                <tr style="border-bottom: 1px solid black;">
                    <td style="text-align: right;"colspan ="3"><strong>Sub Total: </strong></td>
                    <td style="text-align: center;"><strong>{{number_format(collect($receives)->sum('rcvQty'),2, '.', ',')}}</strong></td>
                    <td></td>
                    <td></td>
                    <td style="text-align: right;"><strong>{{number_format(collect($receives)->sum('rcvAmount'),2, '.', ',')}}</strong></td>
                    <td></td>
                </tr>
            </tbody>
        </table> 
        </div>
        <div class="footer" style="margin-left:-45px; font-size: x-small;">
            <div class="sig">
                <p>________________________<br> Submitted By</p>
            </div>
            <div class="sig">
                <p>________________________<br> Recommended By</p>
            </div>
            <div class="sig">
                <p>_________________________<br> Approved By</p>
            </div>
        </div>
