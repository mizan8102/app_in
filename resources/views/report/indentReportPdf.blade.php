...
<body>
    <div class="row" style="none">
        ...
        <table width="100%" style="display:block; margin-top: 10px;">
            ...
            <tbody>
            @if ($indents && count($indents) > 0) 
                @foreach($indents as $key=>$indent)
                <tr>
                    <td style="text-align: center;" width="8%">{{$key+1 ?? ''}}</td>
                    <td style="text-align: left;" width="30%">{{$indent->itemName  ?? ''}}</td>
                    <td style="text-align: left;" width="30%">{{$indent->itemNameBn  ?? ''}}</td>
                ...
                <td style="text-align: center;"><strong>{{ number_format(collect($indents)->sum('indentQty'), 2) }}</strong></td>
                <td></td>
            </tr>
            </tbody>
        </table>
