...
<div class="row" style="none">
    <div class="column1">
        ...
        <img class="logo" src="http://localhost:4173/assets/logo.14158b36.png" alt="">
    </div>
    <div class="column3">
...
<div style="margin-left:5px; margin-top:20px; display:block;">
    <div class="report_params_areas" style="float:left;height:20px;width:50%">
        <span style="text-align: right; font-size:12px"><b> Date: </b>{{date('d-m-Y',strtotime($deposit_date)) }}</span>
    </div>
...
<div style="display:block; margin-top:10px;">
    <table width="100%">
        ...
        <tr style="background-color: rgb(3,73,91);">
            ...
            <th style="text-align: center; color: rgb(255,255,255);">Sales Point</th>
            <th style="text-align: center; color: rgb(255,255,255);">Sales Amount(BDT)</th>
            <th style="text-align: center; color: rgb(255,255,255);">Deposited Amount(BDT)</th>
            <th style="text-align: center; color: rgb(255,255,255);">Deposited By</th>
        ...
        @foreach($data as $key => $daily)
            <tr>
                ...
                <td style="text-align: center;">{{$daily->id}}</td>
                <td style="text-align: center;">{{Date('d-m-Y',strtotime($daily->deposit_date))}}</td>
                <td style="text-align: left;">{{$daily->sales_point}}</td>
                <td style="text-align: right;">{{number_format($daily->total_sell, 2, '.', ',')}}</td>
                <td style="text-align: right;">{{number_format($daily->deposit_amount, 2, '.', ',')}}</td>
                <td style="text-align: left;">{{$daily->deposited_by }}</td>
                <td style="text-align: left;">{{$daily->remarks}}</td>
            </tr>
        @endforeach
        @else
            <tr><td colspan="8" class="text-center">No transactions are made on this date{{date('d-m-Y',strtotime($deposit_date)) }} </td>
            </tr>
        ...
        <tr style="border-bottom: 1px solid black;">
            ...
            <td style="text-align: right;"><strong>{{number_format(collect($data)->sum('total_sell'),2, '.', ',')}}</strong></td>
            <td style="text-align: right;"><strong>{{number_format(collect($data)->sum('deposit_amount'),2, '.', ',')}}</strong></td>
            <td ></td>
            <td></td>
        </tr>
        </tbody>
