...
<body>
<form method="GET"  action="{{url('A_02-pdf')}}"
   @csrf
<div class="card">
    <div class="container">
        <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="restuarentID" class="form-select">
                            ...
                            @foreach($resIds as $data)
                                <option value="{{$data->restId}}">{{$data->restName}}</option>
                            @endforeach
                        </select>
                    </div>
                </div> 
...
@if(request('fromDate') && request('toDate') && request('restuarentID')) 
     <h4 class="text-center" style="margin-top:30px; color: rgb(3,73,91);">
      ...
      <p style="text-align: left; font-size: 12px; color:black; margin-left:5px;">
        ...
        <!-- <div class="report_params_areas" style="float:left;height:20px;width:50%">
            <span style="text-align: right; font-size:12px"><b> Restaurant Name: </b>{{$orders[0]->customerName ?? ''}} </span>
        </div> -->
        ...
        <tbody>
            @foreach ($orders as $key => $order)
                <tr>
                    ...
                    <td style="text-align: center;">{{Date('d/m/Y, h:i A',strtotime($order->orderDate))}}</td>
    ...
    <div class="row" id= "item" style="margin-bottom: 3px solid red;">
        <div class="col-md-5">
            ...
            <a href="{{url('/A_02')}}?fromDate={{request('fromDate')}}&toDate={{request('toDate')}}&restuarentID={{request('restuarentID')}}" target="_blank">
