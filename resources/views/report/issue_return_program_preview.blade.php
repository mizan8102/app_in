...
<body>
<form method="GET"  action="{{url('C_05B-pdf')}}"
   @csrf
<div class="card">
    <div class="container">
        <div class="card-body">
            <div class="row">
                <div class="col-md-3">
                    <div class="mb-3">
                    <label><b>Program ID</b></label>
                            <select name="programID" class="form-select">
                                <option value="">Select</option>
                                @foreach($programs as $data)
                                <option value="{{$data->programId}}"{{$data->programId == request('programID') ? 'selected': ''}}>{{$data->ProgramName}}</option>
                                @endforeach
                          </select>
                    </div>
                </div>
...
@if(request('programID')) 
    ...
    <div class="row" id= "item" style="margin-bottom: 3px solid red;">
        <div class="col-md-5">
            ...
            <a href="{{url('C_05B')}}?programID={{ request('programID') }}" target="_blank">
