...
<body>
    <div class="row" style="none">
        ...
        <div style="margin-left:5px; margin-top:20px; display:block;">
            ...
            <div class="report_params_areas" style="float:left;height:20px;width:100%">
                <span style="text-align: right; font-size:12px"><b>Program Name: </b></span>
                <br>
            </div>
        </div>
        ...
        <div style="font-size:x-small">
            <p style="margin-left: 4px; margin-top: 0px;">
                <b> Master Group: </b>{{$data[0]->masterGroupName ?? ''}}<br>
            </p>
        </div>
        ...
        <table width="100%" style="display:block; margin-top: 10px;">
            <thead>
                <tr style="background-color: rgb(3,73,91);">
                    ...
                    <th style="text-align: center; color: rgb(255,255,255);">Total Amount</th>
                    <th style="text-align: center; color: rgb(255,255,255);">Remarks</th>
                </tr>
            </thead>
            <tbody>
            @foreach($data as $key => $indent)
                <tr>
                    <td style="text-align: center;">{{$key+1 ?? ''}}</td>
                    <td style="text-align: left;">{{$indent->iocDetailItemName  ?? ''}}</td>
                    <td style="text-align: left;">{{$indent->iocDetailItemName  ?? ''}}</td>
                    <td style="text-align: center;">{{$indent->conUOM  ?? ''}}</td>
                    <td style="text-align: center;">{{$indent->iocDetailItemName  ?? ''}}</td>
                    <td style="text-align: center;">{{$indent->totalIocAmt  ?? ''}}</td>
            ...
            @endforeach
                <tr style="border-bottom: 1px solid black;">
                    ...
                    <td style="text-align: center;"><strong>{{number_format(collect($data)->sum('totalIocAmt')) ?? ''}}</strong></td>
                    <td></td>
                </tr>
            </tbody>
        </table>
