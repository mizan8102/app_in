...
<body>
<form method="GET"  action="{{url('A_05-pdf')}}"
   @csrf
<div class="card">
    <div class="container">
        <div class="card-body">
...
@if(request('fromDate') && request('toDate')) 
        ...
        <thead>
            <tr style="background-color: rgb(3,73,91);">
                ...
                <th style="text-align: center; color: rgb(255,255,255);">Date</th>
                    ...
                    <th style="text-align: center; color: rgb(255,255,255);">{{$kitchen->kitchenName}}</th>
                    @endforeach
        ...
        <tbody>
            <tr>
                @foreach($kitchens as $kit)
                    ...
                    <td style="text-align: center;">{{Date('d-m-Y',strtotime($kit->sellDate))}}</td>
                @endforeach
                    @foreach($kitchens as $kamount)
                        <td style="text-align: right;">{{number_format($kamount->totalIssueAmount, 2, '.', ',')}}</td>
                    @endforeach
                    <td style="text-align: right;">{{number_format(collect($kitchens)->sum('totalIssueAmount'),2, '.', ',')}}</td>
            </tr>
            <tr style="border-bottom: 1px solid black;">
                <td style="text-align: right;"colspan ="2"><strong>Sub Total: </strong></td>
                    @foreach($kitchensell as $kitchen)
                        <td style="text-align: right;"><strong>{{number_format(collect($kitchen)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>
                    @endforeach
                <td style="text-align: right;"><strong>{{number_format(collect($kitchens)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>
            </tr>
        </tbody>
    </table>
    <div class="row" id= "item" style="margin-bottom: 3px solid red;">
        <div class="col-md-5">
            ...
            <a href="{{url('/A_05')}}?fromDate={{request('fromDate')}}&toDate={{request('toDate')}}" target="_blank">
