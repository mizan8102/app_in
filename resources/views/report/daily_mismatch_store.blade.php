...
<head>
    ...
    <title>Daily Mismatch Sales Store Wise</title>
            ...
            <div class="column3">
                <div class="head_middle">
                    ...
                    <div style="text-align: center; padding-top:-45px; font-size: 12pt;">
                        <p>Daily Mismatch Sales Store Wise <br>
                        </p>
                    </div>
                </div>
            </div>
            <div class="column1">
                <p style="text-align: right; font-size:12px;">Chiklee#</p>
            </div>
        </div>
            ...
            <table width="100%">
                <thead>
                    <tr style="background-color: rgb(3,73,91);">
                        ...
                        <th style="text-align: center; color: rgb(255,255,255);">Store Name</th>
                        <th style="text-align: center; color: rgb(255,255,255);">Transaction Date</th>
                        <th style="text-align: center; color: rgb(255,255,255);">Total Discount</th>
                        <th style="text-align: center; color: rgb(255,255,255);">Total VAT</th>
                ...
                <tbody>
                    @foreach ($mismatches as $key=> $mismatch)
                        <tr>
                            ...
                            <td style="text-align: left;">{{$mismatch->sl_name}}</td>
                            <td style="text-align: center;">{{Date('d/m/Y',strtotime($mismatch->issue_date))}}</td>
                            <td style="text-align: right;">{{number_format($mismatch->total_discount, 2, '.', ',')}}</td>
                            <td style="text-align: right;">{{number_format($mismatch->total_vat_amnt, 2, '.', ',')}}</td>
                            <td style="text-align: right;">{{number_format($mismatch->total_issue_amount_with_vat, 2, '.', ',')}}</td>
                        </tr>
                    @endforeach
                        <tr style="border-bottom: 1px solid black;">
                            <td style="text-align: right;"colspan ="3"><strong>Sub Total: </strong></td>
                            <td style="text-align: right;"><strong>{{number_format(collect($mismatches)->sum('total_discount'),2, '.', ',')}}</strong></td>
                            <td style="text-align: right;"><strong>{{number_format(collect($mismatches)->sum('total_vat_amnt'),2, '.', ',')}}</strong></td>
                            <td style="text-align: right;"><strong>{{number_format(collect($mismatches)->sum('total_issue_amount_with_vat'),2, '.', ',')}}</strong></td>
                        </tr>
                </tbody>
            </table>
        </div>
