...
<body>
<form method="GET"  action="{{url('A_04-pdf')}}"
   @csrf
<div class="card">
    <div class="container">
        <div class="card-body">
...
@if(request('fromDate') && request('toDate')) 
    ...
    <table width="100%" style="margin-top:20px;">
            ...
            <tr style="background-color: rgb(3,73,91);">
                ...
                <th style="text-align: center; color: rgb(255,255,255);">Date</th>
                    @foreach($waiters as $key => $waiter)
                        <th style="text-align: center; color: rgb(255,255,255);">{{$waiter->waiterName}}</th>
                    @endforeach
        ...
        <tbody>
            ...
            <!-- <tr>
                ...
                <td style="text-align: center;">{{Date('d-m-Y',strtotime($waiter->orderDate))}}</td>
                <td style="text-align: right;">{{number_format($waiter->totalIssueAmount, 2, '.', ',')}}</td>
                @endforeach
                <td style="text-align: right;">{{number_format(collect($waiters)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>  
            </tr> -->
            <tr>
                @foreach($waiters as $key => $waiter)
                    <td style="text-align: center;">{{$key+1}}</td>
                    <td style="text-align: center;">{{Date('d-m-Y',strtotime($waiter->orderDate))}}</td>
                @endforeach
                @foreach($waiters as $test)
                    <td style="text-align: right;">{{number_format($test->totalIssueAmount, 2, '.', ',')}}</td>
                @endforeach
                    <td style="text-align: right;">{{number_format(collect($waiters)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>
            </tr>
            <tr style="border-bottom: 1px solid black;">
                <td style="text-align: right;"colspan ="2"><strong>Sub Total: </strong></td>
                    @foreach($grouped as $waiter)
                            <td style="text-align: right;"><strong>{{number_format(collect($waiter)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>
                    @endforeach
                <td style="text-align: right;"><strong>{{number_format(collect($waiters)->sum('totalIssueAmount'),2, '.', ',')}}</strong></td>
            </tr>
        </tbody>
    </table>
    <div class="row" id= "item" style="margin-bottom: 3px solid red;">
        <div class="col-md-5">
            ...
            <a href="{{url('/A_04')}}?fromDate={{request('fromDate')}}&toDate={{request('toDate')}}" target="_blank">
