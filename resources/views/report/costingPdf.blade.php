...
<head>
    ...
    <title>Costing Consumption C_01C</title>
...
<style type="text/css">
    ...
    <body>
        <div class="row" style="none">
            ...
            <div class="column3">
                <div class="head_middle">
                    ...
                    <div style="text-align: center; padding-top:-40px;">
                        <p style ="font-size:14px;">Costing Consumption Summary<br>
            ...
            <div class="column1">
                <p style="text-align: right; font-size:12px;">Chiklee#C_01C</p>
            </div>
        </div>
            <div style="margin-left:5px; margin-top:20px; display:block;">
                ...
                <div class="report_params_areas" style="float:left;height:20px;width:100%">
                    <span style="text-align: right; font-size:12px"><b> Master Group: </b>{{$masters[0]->masterGroupName ?? ''}}</span>
                </div>
            </div>
            @foreach ($groups as $data)
    <div style="font-size:x-small">
        <p style="margin-left: 4px; margin-top: 0px;">
            <b>Group: </b> {{ $data[0]->groupName }}<br>
        </p>
    </div>
            ...
            <tbody>
                @foreach ($data as $key => $store)
                    <tr>
                        ...
                        <td style="text-align: left;">{{$store->iocNo}}</td>
                        <td style="text-align: center;">{{Date('d/m/Y',strtotime($store->effectiveDate))}}</td>
                        <td style="text-align: left;">{{$store->itemName}}</td>
                        <td style="text-align: center;">{{$store->uomCode}}</td>
                        <td style="text-align: right;">{{number_format($store->iocAmount, 2, '.', ',')}}</td>
                        <td style="text-align: left;"></td>
                    </tr>
                @endforeach
                <tr style="border-bottom: 1px solid black;">
                    <td style="text-align: right;"colspan ="5"><strong>Sub Total: </strong></td>
                    <td style="text-align: right;"><strong>{{number_format(collect($data)->sum('iocAmount'),2, '.', ',')}}</strong></td>
                    <td></td>
                </tr>
            </tbody>
        </table>
